...
const WAQI_API_KEY = '92e34881f8b126f6533a33af5d332eab2d2eac6c'; // Replace with your WAQI API key
  ...
  try {
    ...
    if (response.data && response.data.data) {
      ...
      return {
        ...
        aqi: data.aqi,
        intensity: Math.min(data.aqi / 500, 1.0), // Normalize to 0-1
...
async function fetchIndiaAQI() {
  try {
    console.log('Starting India AQI data fetch...');
    
    // Get all stations in India
    const stations = await getIndiaStations();
    
    if (stations.length === 0) {
      console.log('No stations found. Trying alternative method...');
      // Alternative: Use major Indian cities
      const majorCities = [
